...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const waterBtn = document.getElementById('ambient-water');
  const binauralBtn = document.getElementById('ambient-binaural');
  const natureBtn = document.getElementById('ambient-nature');
  const volumeSlider = document.getElementById('ambient-volume');
  const infoBtn = document.getElementById('ambient-info-btn');
  const infoModal = document.getElementById('ambient-info-modal');
  const closeInfoBtn = document.getElementById('close-ambient-info');
  
  // Update button states from saved preferences
  const updateButtonStates = () => {
    if (waterBtn) waterBtn.classList.toggle('active', AmbientSounds.soundStates.water);
    if (binauralBtn) binauralBtn.classList.toggle('active', AmbientSounds.soundStates.binaural);
    if (natureBtn) natureBtn.classList.toggle('active', AmbientSounds.soundStates.nature);
    if (volumeSlider) volumeSlider.value = AmbientSounds.masterVolume * 100;
  };
  
  // Toggle water sound
  if (waterBtn) {
    waterBtn.addEventListener('click', () => {
      AmbientSounds.toggle('water');
      waterBtn.classList.toggle('active', AmbientSounds.soundStates.water);
    });
  }
  
  // Toggle binaural sound
  if (binauralBtn) {
    binauralBtn.addEventListener('click', () => {
      AmbientSounds.toggle('binaural');
      binauralBtn.classList.toggle('active', AmbientSounds.soundStates.binaural);
    });
  }
  ...
  if (natureBtn) {
    natureBtn.addEventListener('click', () => {
      AmbientSounds.toggle('nature');
      natureBtn.classList.toggle('active', AmbientSounds.soundStates.nature);
    });
  }
  ...
  if (volumeSlider) {
    volumeSlider.addEventListener('input', (e) => {
      AmbientSounds.setVolume(e.target.value / 100);
    });
  }
